...
# --- COMPLETELY REWRITTEN FUNCTION (EXPANDED URL LOGIC) ---
def _run_looker_query(
    ...
    try:
        # 1. Create the query payload (needed for BOTH run_inline and URL params)
        query_payload = {
            "model": MODEL_NAME,
            "view": EXPLORE_NAME,
            "fields": fields,
        ...
        # 2. Run the query inline to get the data (for the text answer)
        ...
        # 3. Build the Expanded URL parameters (as per your PDF, Sections 3 & 4)
        url_params = {}
        ...
        # Convert filter dict {'key': 'val'} to {'f[key]': 'val'} (per Section 4.1)
        filter_params = {}
        ...
        base_url = SDK.auth.settings.base_url.rstrip('/') # Remove trailing slash
...
# --- TOOL DEFINITION (Unchanged, description is correct) ---
